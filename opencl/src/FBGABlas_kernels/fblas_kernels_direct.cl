#pragma OPENCL EXTENSION cl_intel_channels : disable 
...
__kernel void ddot(__global double* vector_x, __global double* vector_y, __global double* out, int N) {
    __constant uint SHIFT_REG = 64;

    double acc_o = 0;
    if (N > 0) {
        const int outer_loop_limit = 1 + (N - 1) / SHIFT_REG; // ceiling
        double x, y;
...
__kernel void daxpy(__global double* vector_x, __global double* vector_y, __global double* out, const double alpha, const double beta, int N) {
    __constant uint SHIFT_REG = 64;
    ...
    for (int i = 0; i < outer_loop_limit; i++) {
        // Receive SHIFT_REG elements from the input arrays
        #pragma unroll
        for (int j = 0; j < SHIFT_REG; j++) {
            int idx = i * SHIFT_REG + j;
            if (idx < N) {
                res = alpha * vector_x[idx] + beta * vector_y[idx];
                // Write the data to the output array
                out[idx] = res;
            }
        }
    }
...
__kernel void dgemv( __global const double* A, __global const double* x, __global double* y, const int row_streamed, const int N, const int M, const double alpha, const double beta) {
