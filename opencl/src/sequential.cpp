...
void Sequential::conjugate_gradient(const double * A, const double * b, double * x, size_t size, int max_iters, double rel_error) const
{
    ...
    {
        std::cout << "Converged in" << num_iters << ", relative error is: " <<  std::sqrt(rr / bb) << std::endl;
    }
    else
    {
        std::cout << "Did not converge in" << num_iters << ", relative error is: " <<  std::sqrt(rr / bb) << std::endl;
    }
